...
const src = join(here, '..', '..', 'backend', 'static')
const dst = join(here, '..', 'public')

if (!existsSync(join(src, 'wayanad.pmtiles'))) {
  console.error(`Missing ${join(src, 'wayanad.pmtiles')} — run from the sankat-mochan repo (backend/static).`)
  process.exit(1)
}
...
console.log('map assets copied to app-simulator/public/')
