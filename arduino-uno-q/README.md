...
| `sync-from-raspberrypi.sh` | your dev machine | refreshes `field-node/` from the canonical `raspberrypi/` |
...
reaches the on-board MCU). Copy this whole `arduino-uno-q/` folder to the UNO Q
(e.g. `scp -r arduino-uno-q unoq:~/`), then, on the board:

```bash
arduino-app-cli app start ~/arduino-uno-q     # builds sketch/ + flashes the MCU + keeps it alive
...
`run.sh` creates a venv in `arduino-uno-q/.venv`, installs `bleak` + `requests` + `pyserial`
...
`field-node/` is a **deployment copy** of `raspberrypi/` (the source of truth). If you change
the mesh code in `raspberrypi/`, refresh this copy before re-deploying:

```bash
./sync-from-raspberrypi.sh
```
...
| pre-flight: `modem did not answer over the Router Bridge` | flash + start the App: `arduino-app-cli app start ~/arduino-uno-q`, then check the MCU with `arduino-app-cli monitor` |
