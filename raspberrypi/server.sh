...
#
# The command post is a FastAPI app (backend/app.py) served by uvicorn. The gateway
# is raspberrypi/run.sh, which does its own pre-flight on the radios.
#
...
# This script lives in raspberrypi/; the repo root is one level up. The shared venv and
# logs stay at the repo root so existing Pi deployments keep their provisioned .venv.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO="$(dirname "$ROOT")"
VENV="$REPO/.venv"
PY="$VENV/bin/python"
POST_DIR="$REPO/backend"
LOG_DIR="$REPO/logs"
PORT="${PORT:-9000}"
HOST="${HOST:-0.0.0.0}"
...
# raspberrypi/run.sh expects. Both scripts share this one venv on purpose.
step "Python environment"
# uv is the project standard (deps live in backend/pyproject.toml). Only resync when the
# pyproject actually changes — an install run per boot is slow on a Pi. If uv is missing
# but a provisioned venv already exists, run with it as-is; the import check below is the
# gate that matters.
REQ="$POST_DIR/pyproject.toml"
STAMP="$VENV/.pyproject.sha"
if command -v uv >/dev/null 2>&1; then
  if [[ ! -x "$PY" ]]; then
    echo "  creating venv at $VENV (uv)"
    uv venv --system-site-packages "$VENV" || die "could not create the venv"
  fi
  if [[ ! -f "$STAMP" ]] || ! sha256sum -c --status "$STAMP" 2>/dev/null; then
    echo "  installing backend dependencies with uv (first run, or they changed)"
    uv pip install --quiet --python "$PY" -r "$REQ" || die "uv could not install from $REQ"
    sha256sum "$REQ" > "$STAMP"
  else
    echo "  requirements already satisfied"
  fi
else
  [[ -x "$PY" ]] || die "uv is not installed and no venv exists at $VENV.
  Install uv first:  curl -LsSf https://astral.sh/uv/install.sh | sh"
  echo "  uv not found — using the existing venv at $VENV as-is"
fi
...
radios_busy() {
  pgrep -f "raspberrypi/gateway\.py" >/dev/null 2>&1
}
...
if [[ "$MODE" == "gateway" || "$MODE" == "local" ]]; then
  ...
  Is uvicorn running there, and is the port open through its firewall?
    on the Mac:  cd backend && uv run uvicorn app:app --host 0.0.0.0 --port 9000
...
$(pgrep -af "raspberrypi/gateway\.py" | sed 's/^/    /')
  stop it first:  kill $(pgrep -f "raspberrypi/gateway\.py" | tr '\n' ' ')"
  fi
  ...
  supervise gateway "$LOG_DIR/gateway.log" "$ROOT" "$C_GRN" "$ROOT/run.sh" &
  SUPERVISORS+=($!)
fi
