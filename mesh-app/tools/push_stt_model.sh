...
ART="$(cd "$(dirname "$0")/../../command-post/aihub_out/artifacts/indic_ctc_precompiled" && pwd)"
DEST="/sdcard/Android/data/${PKG}/files/stt"
VENV_PY="$(cd "$(dirname "$0")/../../command-post" && pwd)/.venv/bin/python"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
push_graph() {
  ...
  if [[ -z "$zip" ]]; then
    echo "!! missing artifact for '$name' in $ART/$name — run command-post/aihub_precompiled_stt.py first"
    exit 1
  fi
  ...
  # onnxruntime-android accepts it. Uses the command-post venv's onnx.
  "$VENV_PY" - "$onnx" <<'PY'
import sys, onnx
