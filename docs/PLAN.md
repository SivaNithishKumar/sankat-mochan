...
- **Android app** (`mobile-application/`, native Kotlin): one app, 3 roles (Victim/Responder/
...
- **Command post** (`backend/`, FastAPI): `POST /sos` ingest + dedup, WebSocket
...
- *Script: `backend/aihub_precompile.py` (to be written).*
...
- [ ] Write `backend/npu_server.py` — FastAPI OpenAI-compatible shim around
