...
```bash
# 1) Install torch for YOUR platform first (see https://pytorch.org), e.g.:
uv venv && uv pip install torch          # CPU build; use the CUDA index-url on a GPU box
# 2) Then the rest (from pyproject.toml):
uv pip install -r pyproject.toml
# 3) Gemma is gated — accept its terms on HF, then:
